...
void I_StartupSound(void)
{
	...
	if(Mix_OpenAudio(SAMPLERATE,AUDIO_S16SYS,1,BUFFERSIZE)<0)
	{
		printf("SDL mixer: %s\n",Mix_GetError());
...
static Mix_Chunk *ds2chunk(void *stream)
{
	...
	sound=Z_Malloc(newsamples*sizeof(Sint16),PU_SOUND,NULL);
	s=(UINT8 *)stream;
	d=sound;
	...
	for(i=0;i<newsamples;i++)
	{
		...
		*d++=(Sint16)(((INT32)s[n]-128)*256);
	}

	return Mix_QuickLoad_RAW((Uint8 *)sound,
		(Uint32)((UINT8 *)d-(UINT8 *)sound));
}
...
INT32 I_StartSound(sfxenum_t id,UINT8 vol,UINT8 sep,UINT8 pitch,
	UINT8 priority,INT32 channel)
{
	UINT8 volume;
	INT32 handle;

	if(!sound_started||!S_sfx[id].data)
		return -1;
...
{
	return (UINT32)(loop_point*1000.0);
}


boolean I_SetSongPosition(UINT32 p)
{
	return music&&Mix_SetMusicPosition((double)p/1000.0)==0;
}
...
boolean I_LoadSong(char *data,size_t len)
{
	...
	if(!data||len<4||memcmp(data,"OggS",4))
		return false;
...
boolean I_FadeSongFromVolume(UINT8 target,UINT8 source,UINT32 ms,
	void (*callback)(void))
{
	(void)source;
	(void)ms;
